...
function zoomPreload(B) {
	...
	if (imgPreload.src.indexOf(B.getAttribute("href").substr(
			B.getAttribute("href").lastIndexOf("/"))) == -1) {
		preloadActive = true;
...
function preloadAnimStart() {
	...
	document.getElementById("ZoomSpin").style.top = ((myHeight / 2) + myScroll)
			+ "px";
	document.getElementById("ZoomSpin").style.visibility = "visible";
	preloadFrame = 1;
	document.getElementById("SpinImage").src = zoomImageURI + "zoom-spin-"
			+ preloadFrame + ".png";
...
function zoomIn(A) {
	...
	if (zoomActive[theID] != true) {
		...
		zoomTimer[theID] = setInterval("zoomElement('" + zoomID + "', '"
				+ theID + "', " + zoomCurrent + ", " + startW + ", "
				+ zoomChangeW + ", " + startH + ", " + zoomChangeH + ", "
				+ hostX + ", " + zoomChangeX + ", " + hostY + ", "
				+ zoomChangeY + ", " + zoomSteps + ", " + fade + ", "
				+ fadeAmount + ", 'zoomDoneIn(zoomID)')", zoomTime);
...
function zoomOut() {
	if (zoomActive[theID] != true) {
		...
		zoomTimer[theID] = setInterval("zoomElement('" + zoomID + "', '"
				+ theID + "', " + zoomCurrent + ", " + startW + ", "
				+ zoomChangeW + ", " + startH + ", " + zoomChangeH + ", "
				+ startX + ", " + zoomChangeX + ", " + startY + ", "
				+ zoomChangeY + ", " + zoomSteps + ", " + fade + ", "
				+ fadeAmount + ", 'zoomDone(zoomID, theID)')", zoomTime);
		zoomActive[theID] = true
	}
}
...
function zoomElement(zoomdiv, theID, zoomCurrent, zoomStartW, zoomChangeW,
		zoomStartH, zoomChangeH, zoomStartX, zoomChangeX, zoomStartY,
		zoomChangeY, zoomSteps, fade, fadeAmount, execWhenDone) {
	if (zoomCurrent == (zoomSteps + 1)) {
		...
		zoomCurrent++;
		clearInterval(zoomTimer[theID]);
		zoomTimer[theID] = setInterval("zoomElement('" + zoomdiv + "', '"
				+ theID + "', " + zoomCurrent + ", " + zoomStartW + ", "
				+ zoomChangeW + ", " + zoomStartH + ", " + zoomChangeH + ", "
				+ zoomStartX + ", " + zoomChangeX + ", " + zoomStartY + ", "
				+ zoomChangeY + ", " + zoomSteps + ", " + fade + ", "
				+ fadeAmount + ", '" + execWhenDone + "')", zoomTime)
	}
}
...
function fadeElementSetup(C, E, B, A, D) {
	if (fadeActive[C] == true) {
		...
		fadeTimer[C] = setInterval("fadeElement('" + C + "', '" + fadeCurrent
				+ "', '" + fadeAmount + "', '" + fadeSteps + "')", 15);
		fadeActive[C] = true;
		if (D == 1) {
			fadeClose[C] = true
		} else {
...
function fadeElement(B, D, A, C) {
	if (D == C) {
		...
		if (fadeQueue[B] && fadeQueue[B] != false) {
			fadeElementSetup(fadeQueue[B][0], fadeQueue[B][1], fadeQueue[B][2],
					fadeQueue[B][3]);
			fadeQueue[B] = false
		}
	} else {
		...
		clearInterval(fadeTimer[B]);
		fadeTimer[B] = setInterval("fadeElement('" + B + "', '" + D + "', '"
				+ A + "', '" + C + "')", 15)
	}
}
...
function getSize() {
	if (document.all) {
		myWidth = (document.documentElement.clientWidth) ? document.documentElement.clientWidth
				: document.body.clientWidth;
		myHeight = (document.documentElement.clientHeight) ? document.documentElement.clientHeight
				: document.body.clientHeight;
		myScroll = (document.documentElement.scrollTop) ? document.documentElement.scrollTop
...
function scrollVertAnim() {
	if (scrollanim.time > scrollanim.duration) {
		...
		move = sineInOut(scrollanim.time, scrollanim.begin, scrollanim.change,
				scrollanim.duration);
...
function showLargePopup(B) {
	...
	refreshTimer = setTimeout("setLocation('" + B.getAttribute("href") + "')",
			1500)
}
...
function showDownloadPopup(C) {
	...
	if (dpopTimer != "") {
		...
		moveStart(A, parseInt(A.style.left), parseInt(A.style.left),
				parseInt(A.style.top) + 10, parseInt(A.style.top), 15);
...
function actuallyHide() {
	...
	if (dpopTimer != "") {
		dpopTimer = "";
		moveStart(A, parseInt(A.style.left), parseInt(A.style.left),
				parseInt(A.style.top), parseInt(A.style.top) - 10, 15);
...
function moveAnimDo() {
	if (moveanim.time > moveanim.duration) {
		...
		moveX = cubicOut(moveanim.time, moveanim.beginX, moveanim.changeX,
				moveanim.duration);
		moveY = cubicOut(moveanim.time, moveanim.beginY, moveanim.changeY,
				moveanim.duration);
...
G.SO = {
	initData : {
		web : [
				...
				[
						...
						[ "好搜", "http://www.haosou.com/s?q={key}" ],
						[ "chinaSo", "http://www.chinaso.com/search/pagesearch.htm?q={key}" ],
						//[ "谷歌1", "http://www.xiexingwen.com/google/sp.php?from=sug&query={key}" ],
						[ "谷歌", "http://scholar.glgoo.org/scholar?hl=zh-CN&btnG=&lr=&q={key}" ],
						//[ "谷歌3", "https://www.ggncr.com/search?newwindow=1&safe=strict&q={key}" ],
						[ "谷歌1", "http://www.aigoogle.xyz/prevgoogle/response.php?query={key}" ],
						[ "搜狗", "http://www.sogou.com/web?query={key}&_ast=1234423248" ] ] ],
		widget : [
				"widget",
				"软件",
				[
						//[ "快搜","http://ks.pconline.com.cn/download.shtml?q={key}&encode=utf" ],
						[ "华军","http://search.newhua.com/search_list.php?searchsid=1&searchname={key}" ],
						//[ "绿茶","http://www.33lc.com/index.php?q={key}" ],
						//[ "非凡", "http://search.crsky.com/search.aspx?keyword={key}&sType=all" ],
						[ "PCHome","http://download.pchome.net/search-{key}---0-1.html" ]
						 ] ],
		video : [
				"video",
				"视频",
				[
						...
						//[ "56", "http://so.56.com/index?key={key}" ],
						[ "百度", "http://video.baidu.com/v?ct=301989888&rn=20&pn=0&db=0&s=0&fbl=800&ie=utf-8&word={key}" ],
						[ "雅虎", "http://video.youdao.com/search?q={key}" ],
		...
		mp3 : [
				...
				[
						...
						["好搜", "http://s.music.haosou.com/s?frsug=0&q={key}" ],
						["全网搜", "http://www.quanss.com/w_mp2.htm#{key}" ],
		...
		pic : [
				...
				[
						...
						[ "好搜", "http://image.haosou.com/i?q={key}&src=srp" ],
						[ "妹子", "http://www.mzitu.com/search/{key}" ],
						[ "有道", "http://image.youdao.com/search?q={key}" ],
						[ "搜狗", "http://pic.sogou.com/pics?query={key}" ],
						[ "必应", "http://cn.bing.com/images/search?q={key}" ] ] ],
		book : [
				"book",
				"图书",
				[
						[ "豆丁",
								"http://www.docin.com/app/searchnew/findproduct?keyword={key}{encode}" ],
						[ "百度", "http://www.baidu.com/baidu?q1={key}&ft=all" ],
						[ "搜狗",
								"http://www.sogou.com/web?query=filetype%3Aall+{key}&filetype=all" ],
						[ "爱问",
								"http://ishare.iask.sina.com.cn/search.php?key={key}&format=" ],
						
						[ "起点",
								"http://sosu.qidian.com/searchresult.aspx?keyword={key}" ] ] ],
		map : [
				"map",
				"地图",
				[
						[ "我要地图", "http://www.51ditu.com/maps?wd={key}" ],
		...
		news : [
				...
				[
						...
						[ "百度",
								"http://news.baidu.com/ns?cl=2&rn=20&tn=news&word={key}" ],
		...
		know : [
				...
				[
						[
								"百度",
								"http://zhidao.baidu.com/q?word={key}&ct=17&pn=0&tn=ikaslist&rn=10&lm=0&fr=search" ],
						[ "天涯",
								"http://wenda.tianya.cn/search.jsp?q={key}" ],
						[
								'搜搜',
								'http://wenwen.soso.com/z/SearchSolved.e?sp=S{key}&w={key}&search=%E6%90%9C%E7%B4%A2%E7%AD%94%E6%A1%88{encode}' ] ] ]
	},
	init : function() {
		var H = G.SO.initData;
		...
		for ( var B in H) {
			try {
				var E = H[B];
				F += "<li" + (B == 0 ? " class=active" : "") + " id=" + E[0]
						+ '-tab><a onfocus="blur();" onclick="ScrollSection(\''
						+ E[0] + "-pane', 'scroller', '" + H.web[0]
						+ '-pane\'); return false" href="javascript:void(0);">'
						+ E[1] + "</a></li>";
				var A = E[2];
				if (!A) {
					continue
				}
				D += "<div class=section id="
						+ E[0]
						+ '-pane><div class="hbi_cont hbi_search"><div class="searchBox"><ul class="searchEngine">';
				for ( var C = 0; C < A.length; C++) {
					D += '<li><a id="'
							+ E[0]
							+ C
							+ '"'
							+ (C == 0 ? ' class="currentEngine"' : "")
							+ ' href="javascript:void(0);" onfocus="blur();" onclick="G.SO.change(\''
							+ E[0] + "'," + C + ');return false;">' + A[C][0]
							+ "</a></li>"
				}
				D += '</ul><input type="hidden" id="'
						+ E[0]
						+ '_hid" value="'
						+ A[0][1]
						+ '"/><div class="searchCont"><input id="'
						+ E[0]
						+ '_txt" type="text" text="请输入搜索关键字" class="text" onkeydown="if(event.keyCode == 13){this.nextSibling.onclick();}" onfocus="this.select();"/><img src="style/images/search_image.gif" onclick="G.SO.go(\''
						+ E[0] + "');\"/></div></div></div></div>"
			} catch (I) {
				alert(I)
			}
	...
	go : function(B) {
		...
		C = A.replace(/\{key\}/g, C);
		if (B == "widget") {
			C = C.replace(/\{r\}/, [ "A", "B", "C", "D", "E", "F", "G" ][Math
					.floor(Math.random() * 5)])
		}
		window.open(C)
	}
...
G.SO.init();
setTimeout(function() {
	ScrollSection("web-pane", "scroller", "widget-pane")
}, 100);